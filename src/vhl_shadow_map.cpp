...
namespace vhl
{
    ...
    VhlShadowMap::VhlShadowMap(VhlDevice& device, int width) : m_Device{device}
    {
        m_ShadowMapExtent.width = width;
        m_ShadowMapExtent.height = width;
        //m_DepthFormat = m_Device.findDepthFormat();
        m_DepthFormat = offscreenDepthFormat;
    ...
    void VhlShadowMap::createShadowRenderPass()
    {
        // ATTACHMENT_DEPTH
        ...
        // Index of the render pass being depended upon by dstSubpass
        dependencies[0].srcSubpass = VK_SUBPASS_EXTERNAL;
        // The index of the render pass depending on srcSubpass 
        dependencies[0].dstSubpass = static_cast<uint32_t>(SubPassesShadow::SUBPASS_SHADOW); 
        ...
        // What access scopes influence the dependency
        ...
        {
            throw std::runtime_error("failed to create render pass! (CreateShadowRenderPass)");
        }
    }

    void VhlShadowMap::createShadowDepthResources()
    {
        // image
        ...
        if (result != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create texture image view! (CreateShadowDepthResources)");
        }
        ...
        result = vkCreateSampler(m_Device.device(), &samplerCreateInfo, nullptr, &m_ShadowDepthSampler);
        if (result != VK_SUCCESS)
        {
    ...
    void VhlShadowMap::createShadowFramebuffer()
    {
